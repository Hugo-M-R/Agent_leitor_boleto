"""Rotas de extração para upload e leitura por caminho.

Este módulo fornece:
- POST /extract: recebe um arquivo (PDF/Imagem/TXT), extrai e salva JSONs em `retornos/`.
- POST /extract/by-path: recebe `{ path }` e realiza a extração.
- GET /extract/returns: lista arquivos JSON gerados.
- GET /extract/returns/file: retorna o conteúdo de um JSON específico com validação de path.
"""
...
class ExtractResponse(BaseModel):
	"""Resposta padrão de extração contendo campos do boleto e metadados de saída."""
...
def _ensure_returns_dir() -> str:
	"""Garante a existência do diretório `retornos/` e retorna seu caminho absoluto."""
...
def _save_transcription_if_new(base_dir: str, text: str) -> Optional[str]:
	"""Salva a transcrição se o texto for inédito, retornando o caminho do arquivo."""
	if not text:
		return None
...
async def extract_upload(file: UploadFile = File(...)):
	"""Recebe upload de arquivo, extrai informações e persiste extracao/transcricao."""
...
async def extract_by_path(payload: ByPathRequest = Body(...)):
	"""Extrai informações a partir de um caminho de arquivo local (ou em `dados/`)."""
...
async def list_returns():
	"""Lista arquivos de retorno presentes no diretório `retornos/`."""
...
async def get_return_file(path: str = Query(..., description="Caminho absoluto para arquivo em retornos")):
	"""Retorna o conteúdo de um JSON em `retornos/`, protegendo contra path traversal."""
