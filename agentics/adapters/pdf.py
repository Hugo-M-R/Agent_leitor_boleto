"""Leitura de PDFs e OCR com múltiplas estratégias e fallbacks.

Ordem tentada para extrair texto:
1. PyPDF2 (se houver camada de texto válida)
2. pdfminer.six
3. ocrmypdf + pdfminer/fitz (gera camada de texto via Tesseract)
4. Tesseract por página (pdf2image) com pré-processamento
5. EasyOCR (CPU)

Também expõe utilitário para converter páginas de PDF em imagens.
"""
...
def _pdf_pages_to_images_pymupdf(path: str, dpi: int) -> List[Any]:
    """Converte páginas de um PDF em imagens usando PyMuPDF (fitz)."""
...
def _has_text_in_pdf(path: str) -> bool:
    """Verifica rapidamente se a primeira página possui camada de texto útil."""
    try:
...
def _read_pdf_text(path: str) -> str:
    """Extrai texto de PDF com vários fallbacks, retornando string possivelmente vazia."""
...
def pdf_to_images(pdf_path: str, output_dir: str | None = None, dpi: int = 400, format: str = "PNG") -> List[str]:
    """Salva páginas do PDF como imagens no diretório informado (ou temporário)."""
    try:
        settings = get_settings()
