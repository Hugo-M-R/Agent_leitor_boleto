"""Funções de extração e validação de informações de boletos.

Este módulo concentra a heurística para:
- localizar e pontuar datas de vencimento no texto;
- extrair linha digitável, valor, banco emissor, nosso número e beneficiário;
- validar a linha digitável com os algoritmos módulo 10 e 11;
- extrair CNPJ/CPF quando presente.

As funções expostas mais relevantes são:
- extract_due_date(text): retorna um dicionário com due_date, original, confidence;
- extract_payment_info(text): retorna um dicionário consolidado com campos do boleto.
"""
...
def _parse_date(d: str, m: str, y: str) -> Optional[datetime]:
    """Cria um datetime válido a partir de strings de dia/mês/ano.

    Retorna None para datas inválidas (ex.: 31/02/2024).
    """
    try:
...
def _find_all_dates(text: str) -> List[Tuple[datetime, Tuple[int, int]]]:
    """Enumera todas as datas plausíveis no texto e suas posições.

    Busca tanto por datas soltas quanto próximas a palavras-chave de vencimento.
    """
...
def _find_keywords_positions(text: str) -> List[Tuple[str, Tuple[int, int]]]:
    """Retorna posições de palavras-chave relacionadas a vencimento no texto."""
...
def _score_date(span: Tuple[int, int], keyword_spans: List[Tuple[str, Tuple[int, int]]], text: str) -> float:
    """Pontua uma data considerando proximidade e contexto com palavras-chave.

    A presença de uma palavra-chave na mesma linha e à esquerda aumenta a pontuação.
    """
...
def extract_due_date(text: str) -> Optional[Dict[str, str]]:
    """Extrai a melhor data de vencimento do texto.

    Retorna um dicionário com:
    - due_date: string no formato YYYY-MM-DD
    - original: substring original encontrada
...
def _modulo11_codigo_barras(campo: str) -> int:
    """Calcula dígito verificador módulo 11 para código de barras (44 dígitos)."""
    soma = 0
...
def _modulo10_linha_digitavel(campo: str) -> int:
    """Calcula dígito verificador módulo 10 para blocos da linha digitável."""
    soma = 0
    soma_val = 0
    mult = 2
    for d in reversed(campo):
...
def _validate_linha_digitavel(linha: str) -> Dict[str, Any]:
    """Valida a linha digitável (47 dígitos) retornando detalhes e consistência.

    Verifica DV de cada bloco e decodifica fator de vencimento e valor.
    """
...
def _extract_valor_boleto(text: str) -> Optional[float]:
    """Tenta extrair o valor do boleto a partir de rótulos comuns no texto."""
    patterns = [
...
def _extract_banco_emissor(text: str) -> Optional[str]:
    """Extrai o código de banco emissor (três dígitos) se presente."""
...
def _extract_nosso_numero(text: str) -> Optional[str]:
    """Extrai o campo "Nosso Número" quando identificado por rótulo."""
...
def _looks_like_digitable(s: str) -> bool:
    """Heurística para saber se um trecho parece linha digitável (47/48 dígitos)."""
    digits = re.sub(r"\D+", "", s)
    return len(digits) >= 20 or bool(DIGITABLE_REGEX.search(s))


def _is_mostly_digits_or_codes(s: str) -> bool:
    """Retorna True se a string for majoritariamente dígitos/códigos (evita falsos positivos)."""
    digits = sum(ch.isdigit() for ch in s)
    return digits >= max(8, int(0.5 * max(1, len(s))))


def _extract_beneficiario(text: str) -> Optional[str]:
    """Extrai o nome do beneficiário/cedente usando rótulos e contexto de linhas.

    Evita confundir com linha digitável e campos numéricos.
    """
...
def _extract_linha_digitavel(text: str) -> Optional[str]:
    """Localiza a linha digitável no texto, normalizando para apenas dígitos."""
    candidates = []
...
def extract_payment_info(text: str) -> Dict[str, Any]:
    """Extrai e valida campos principais de um boleto a partir de texto OCR/textual.

    Retorna dicionário com: linha_digitavel, data_vencimento, beneficiario, cnpj,
    valor, banco_emissor, nosso_numero, validacoes e flag boleto_valido.
    """
...
def _extract_cnpj(text: str) -> Optional[str]:
    """Extrai CNPJ/CPF formatado, evitando áreas próximas à linha digitável."""
